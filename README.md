...
- 时间轴仅收录离散年度（2000/2005/2010/2015/2020/2023，以 `/api/meta` 为准）。访问未收录年份（如 `?year=2022`）时，请求发出前即吸附到最近年度（2023），并同步纠正全局状态与地址栏，页面顶部给出「2022 年数据暂未收录，展示最近的 2023 年」提示，杜绝“标题 2022 / 数据 2023”的错位；后端对直接 API 调用也采用相同的就近回落。
